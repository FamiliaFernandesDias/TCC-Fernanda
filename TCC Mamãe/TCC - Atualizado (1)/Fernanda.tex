...
% - [ ] a leitura das referências bibliográficas enviadas pelo Prof. Ruben, elaborar com suas PRÓPRIAS PALAVRAS um parágrafo de umas 6 ou 8 linhas (bem bonito) sem citação! como introdução a esse Capitulo!

% - [X] Apaga tudo isso! Foi pedido para você um parágrafo de introdução a esse capítulo!
% - [X] Acrescentar aqui todos os itens considerados neste capítulo (olha o sumário). Complementando o parágrafo de duas linhas colocado no trabalho!

\section{Trilhando pelos caminhos das Metodologias Ativas} \label{sec:2.1_MetAtiv}
...
% Segundo \citeonline[p. 40]{SANTOS2020} ``a experiência deixa marcas, pois desafia o indivíduo a se confrontar com o medo do contato com o diferente, com o receio de ser discriminado e com os riscos inerentes de vivenciar situações novas que não estão a priori sob seu controle''. % Removendo referências ``Não matemáticas''

De acordo com \citeonline{MENDES2019}, a gamificação é compreendida como uma estratégia para ser utilizada nos ambientes formais e não formais de ensino, de modo que ela pode ter suas influências nos processos de ensino e aprendizagem.
...
% A ABP é uma técnica didática pedagógica que se enreda a partir de um problema, para promover luzes a competências e habilidades na resolução do problema, fortalecendo definições imprescindíveis para construções de saberes \cite{SILVA_LINS_LEAO2019}.
...
% Para \citeonline[p. 28]{OLIVEIRA2021} durante o processo que é desencadeado pela Aprendizagem Baseada em Problemas ``o ponto central é a autonomia do estudante estimulado na busca conhecimento por meio da aprendizagem, o que engloba, como principal característica, a relação de conteúdos disciplinares no contexto de um problema orientado''. % Removendo referências ``Não matemáticas''
...
\section{Desafios da implementação das Metodologias Ativas} \label{sec:2.3_DesafiosMetAtiv}
...
Em seu livro, \citeonline{resolucaoDeProblemas2014}, discorrem detalhadamente sobre a integração entre ensino, aprendizagem e avaliação, e como a resolução de problemas se conecta a esses elementos.
...
Agora, para melhor entendermos a Metodologia de Resolução de Problemas, faz-se necessário definir primeiro o que constitui um problema. Em seu trabalho, \citeonline{Romanatto_2012} compila diversas definições de problema.
...
Esse pensamento de superação de desafios é aprofundado por \citeonline{polya1978}, onde traz uma reflexão que diz:
...
Para melhor entender os sólidos geométricos, é importante revisar algumas definições matemáticas básicas. Um sólido geométrico é um objeto tridimensional que possui volume e forma definidos. Os sólidos geométricos são classificados em diferentes categorias, como poliedros e não-poliedros.

Ressalta-se que toda essa subseção se apresenta fortemente baseada nos conceitos matemáticos apresentados por \citeonline{Fundamentos10_2022}, \citeonline{Fundamentos9_2022} e \citeonline{Elon_2022}.

\begin{itemize}
    \item \textbf{1. Sólidos Geométricos}
\end{itemize}

Sólidos geométricos são subconjuntos tridimensionais do espaço euclidiano R3, limitados por superfícies que podem ser planas ou curvas. Esses objetos possuem três dimensões - comprimento, largura e altura - e podem ser classificados em diferentes categorias, como poliedros e corpos redondos.

\begin{itemize}
    \item \textbf{2. Poliedros}
\end{itemize}

Sua definição é dada por \citeonline{Elon_2022}, onde diz que poliedro (\autoref{fig:poliedro}) é uma reunião de um número finito de polígonos planos chamados faces onde:
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.4\textwidth]{Imagens/Novas/Poliedros}
    \legend{Fonte: \citeonline{Elon_2022}}
\end{CenteredFigure}

Todo poliedro (no sentido da definição acima), limita uma região do espaço chamada de interior desse poliedro. Dizemos que um poliedro é convexo se o seu interior é convexo. Vamos recordar o que isto significa.
...
Consideremos em um plano, uma reta $E$ chamada eixo e uma linha $L$, simples, que não corta esse eixo, ambos apresentados na \autoref{fig:Revolução}. Imagine que essa linha ``gire'' em torno do eixo, ou seja, cada ponto $L$ descreva uma circunferência em um plano perpendicular a $E$ e com centro sobre $E$, Cada ponto $P$ e $L$ percorre então uma circunferência cujo raio é a sua distância ao eixo e a reunião de todas essas circunferências é chamada uma superfície de revolução. Se a linha $L$ for fechada ou se seus dois extremos pertencerem ao eixo, a superfície de revolução delimita um sólido chamado sólido de revolução.

\begin{CenteredFigure}
    \caption{Não Poliedro} \label{fig:Revolução}
    \includegraphics[width=0.4\textwidth]{Imagens/Novas/Revolução}
    \legend{Fonte: \citeonline{Elon_2022} - Alterado pela autora}
\end{CenteredFigure}
...
% \cite[Alterado pela autora]{Fundamentos9_2022}

% \end{citacao}

% \subsubsection{Poliedros Convexos} % 2.5.1.2
...
% \cite{Fundamentos10_2022}
% \end{citacao}

% \subsubsection{Poliedros Platônicos} %2.5.1.3
...
% \subsubsection{Prisma: definição, superfícies e volume} %2.5.1.4 % superfície lateral e total do 

\begin{itemize}
    \item \textbf{7. Prisma: definição, superfícies e volume} % superfície lateral e total do}
\end{itemize}
...
\textbf{Prisma convexo limitado} ou \textbf{prisma convexo definido} ou \textbf{prisma convexo} é a reunião da parte do prisma convexo ilimitado, compreendida entre os planos de duas seções paralelas e distintas, com essas seções.
% \end{citacao}
...
% \cite[Marcação da autora]{Fundamentos10_2022}
% \end{citacao}

% \subsubsection{Pirâmide: definição, superfícies e volume} %2.5.1.5 % superfície lateral e total da 
...
A \autoref{fig:elementos piramide} ilustra os elementos de uma pirâmide, sendo eles: suas arestas, altura e triedro. Já a \autoref{fig:apotema} ilustra a apótema de uma pirâmide. Em seu trabalho, \citeonline{Fundamentos10_2022} afirmam que: ``Chama-se \textbf{apótema} de uma pirâmide regular à altura (relativa ao lado da base) de uma face lateral.''

\begin{CenteredFigure}
    \caption{Elementos de uma pirâmide} \label{fig:elementos piramide}
    \includegraphics[width=0.4\textwidth]{Imagens/Novas imagens/elementos_piramide}
    \legend{Fonte: \citeonline{Fundamentos10_2022}}
\end{CenteredFigure}

\begin{CenteredFigure}
    \caption{Apótema de uma pirâmide} \label{fig:apotema}
    \includegraphics[width=0.3\textwidth]{Imagens/Novas imagens/apotema}
    \legend{Fonte: \citeonline{Fundamentos10_2022}}
\end{CenteredFigure}

A \autoref{fig:piramide} ilustra o cálculo de volume de uma pirâmide qualquer, através de sua decomposição em tetraedros.
...
Uma das demonstrações do Teorema de Euler é feita por \citeonline{Zoroastro_2010} onde o mesmo é enunciado da seguinte forma: ``Seja $P$ um poliedro convexo com $F$ faces. $A$ arestas e $V$ vértices. Tem-se necessariamente \textcolor[HTML]{0000FF}{$F - A + V = 2$}.''

Na demonstração deste teorema, feita por \citeonline{Zoroastro_2010}, ele relembra algumas definições que destacaremos abaixo.
...
Em seu trabalho, \citeonline{Zoroastro_2010} divulga sua maneira de demonstrar o Teorema de Euler.
...
\subsection{Encontro 2 - Planificação Poliedros de Platão - Relação de Euler}
...
O Encontro 4 (\autoref{ApendiceE}) tem como objetivo calcular as áreas laterais e totais dos prismas. Para isso cada grupo recebeu uma folha com as instruções a serem seguidas e também uma embalagem no formato de um prisma como descrito abaixo:
...
O Encontro 6 (\autoref{ApendiceG}) tem como objetivo: Calcular o volume do prisma. Cada grupo recebeu o mesmo prisma da aula do cálculo da área lateral e total.
...
\subsection{Análise do Encontro 2}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.8\linewidth]{Novas imagens/250 - Encontro 3 - Questão 9 resposta certa e errada}
    \legend{\autoria}
\end{CenteredFigure}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.5\linewidth]{04-Encontro 4 Embalagens ANTES}
    \legend{\autoria}
\end{CenteredFigure}

\begin{CenteredFigure}
    \caption{Embalagens depois do cálculo da área} \label{fig:5-embalagens depois}
    \includegraphics[width=0.5\linewidth]{05-Encontro 4 - Embalagens DEPOIS}
    \legend{\autoria}
\end{CenteredFigure}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.7\linewidth]{Novas imagens/266 - Encontro 4 - Grupo 1 acertou todos os cálculos da área lateral e total do prisma}
    \legend{\autoria}
\end{CenteredFigure}

% \begin{CenteredFigure}
%     \caption{Encontro 4 - Grupo 2 não acertou todos os cálculos da área lateral e total do prisma} \label{fig: 267 - Encontro 4 - Grupo 2 nao acertou todos os calculos da area lateral e total do prisma}
%     \includegraphics[width=0.5\linewidth]{Novas imagens/267 - Encontro 4 - Grupo 2 não acertou todos os cálculos da área lateral e total do prisma}
%     \legend{\autoria}
% \end{CenteredFigure}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.5\linewidth]{Novas imagens/251 - Encontro 5 - Questão 7 - resposta correta}
    \legend{\autoria}
\end{CenteredFigure}

\begin{CenteredFigure}
    \caption{Encontro 5 - Questão 7 - resposta incorreta} \label{fig: 252 - Encontro 5 - Questao 7 - resposta incorreta}
    \includegraphics[width=0.5\linewidth]{Novas imagens/252 - Encontro 5 - Questão 7 - resposta incorreta}
    \legend{\autoria}
\end{CenteredFigure}
...
\subsection{Análise do Encontro 7}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.5\linewidth]{Novas imagens/254 - Encontro 7 - Questão 5 - resposta incorreta}
    \legend{\autoria}
\end{CenteredFigure}

\begin{CenteredFigure}
    \caption{Encontro 7 - Questão 5 - resposta correta} \label{fig: 253 - Encontro 7 - Questao 5 - resposta correta}
    \includegraphics[width=0.5\linewidth]{Novas imagens/253 - Encontro 7 - Questão 5 - resposta correta}
    \legend{\autoria}
\end{CenteredFigure}
...
\subsection{Análise do Encontro 9}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.4\linewidth]{Novas imagens/255 - Encontro 9 - Questão 4 - resposta correta}
    \legend{\autoria}
\end{CenteredFigure}

\begin{CenteredFigure}
    \caption{Encontro 9 - Questão 4 - resposta incorreta} \label{fig: 256 - Encontro 9 - Questao 4 - resposta incorreta}
    \includegraphics[width=0.4\linewidth]{Novas imagens/256 - Encontro 9 - Questão 4 - resposta incorreta}
    \legend{\autoria}
\end{CenteredFigure}

\subsection{Análise do Encontro 10}
...
Percebeu-se que, o grupo 2 apresentou certa dificuldade para entender e resolver algumas questões, como mostra a \autoref{tab:Acertos do Encontro 11} e a \autoref{fig: 257 - Encontro 11 - Questao 1 - resposta incorreta}. E, na \autoref{fig: 258 - Encontro 11 - Questao 1 - resposta correta}, constata-se o cálculo correto da mesma questão, feito pelo grupo 5.
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.5\linewidth]{Novas imagens/257 - Encontro 11 - Questão 1 - resposta incorreta}
    \legend{\autoria}
\end{CenteredFigure}

\begin{CenteredFigure}
    \caption{Encontro 11 - Questão 1 - cálculo e resposta corretos} \label{fig: 258 - Encontro 11 - Questao 1 - resposta correta}
    \includegraphics[width=0.5\linewidth]{Novas imagens/258 - Encontro 11 - Questão 1 - resposta correta}
    \legend{\autoria}
\end{CenteredFigure}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.5\textwidth]{Novas imagens/259 - Encontro 12 - Colagem incorreta no Álbum de Figurinhas}
    \legend{\autoria}
\end{CenteredFigure}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=0.5\textwidth]{Novas imagens/261 - Encontro 12 - Colagem incorreta no Álbum de Figurinhas}
    \legend{\autoria}
\end{CenteredFigure}
...
\begin{CenteredFigure}
    ...
    \includegraphics[width=\linewidth]{Satisfação dos alunos por encontro - 1}
    \legend{\autoria}
\end{CenteredFigure}

\begin{CenteredFigure}
    \caption{Satisfação dos alunos por encontro - Parte 2} \label{fig:satisfacao2}
    \includegraphics[width=\linewidth]{Satisfação dos alunos por encontro - 2}
    \legend{\autoria}
\end{CenteredFigure}
